/**
 * COMPONENTE: Seção de DCTFs Enviadas Fora do Prazo
 * 
...
export function DCTFsForaDoPrazoSection({ dctfs, loading = false, error = null }: Props) {
  ...
  const handleExportar = async () => {
    ...
    try {
      ...
      const data = dctfs.map((dctf) => [
        ...
        formatDate(dctf.data_vencimento),
        dctf.dias_atraso.toString(),
        dctf.situacao || '—',
        dctf.tipo || '—',
      ...
      await exportToExcel({
        filename: `dctfs-fora-do-prazo-${new Date().toISOString().split('T')[0]}.xlsx`,
        sheetName: 'DCTFs Fora do Prazo',
        headers: ['Empresa', 'CNPJ', 'Período Apuração', 'Data Transmissão', 'Data Vencimento', 'Dias de Atraso', 'Situação', 'Tipo', 'Severidade', 'Mensagem'],
        data,
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border border-red-200 p-6">
        <div className="flex items-center gap-2 text-red-600">
          ...
          <p className="font-medium">Erro ao carregar DCTFs fora do prazo</p>
        </div>
                      ...
                      <tr key={dctf.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm font-medium text-red-600">
                            {dctf.dias_atraso} dia{dctf.dias_atraso !== 1 ? 's' : ''}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
